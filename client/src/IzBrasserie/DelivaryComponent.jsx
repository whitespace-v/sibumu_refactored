...
const DelivaryComponent = (props) => {
    ...
    return (
        <motion.div className={props.element.style_delivery} whileInView={'visible'} initial={'notVisible'}
            ...
            <motion.div className="delivery_elem_izBrasserie" variants={variantImg} transition={()=>transition_option(props.element.id)}>
                <div className="picture_object_izBrasserie">
                    <img className="scooter" alt="scooter_image" src={props.element.img}/>
                </div>
                <div className="text_delivery_isBrasserie">{props.element.text}</div>
            </motion.div>
        </motion.div>
    );
};
