...
const HistoryAndLegend = ({ title, text, legend, menu }) => {
	...
	if (legend === 0) {
		return (
			<motion.div className="text_legend_history" whileInView={'visible'} initial={'notVisible'}
				viewport={{ once: true, amount: 0.1 }} transition={{ duration: 1.5 }}>
					<div className="title_history">{title}</div>
					<div className="text_history">{text}</div>
					<a className='download-catalog' href={menu} download>Скачать меню в пдф</a>
			</motion.div>
		);
	}
	else {
		return (
			<motion.div className="legendText" whileInView={'visible'} initial={'notVisible'}
				...
				<div className="black text_history">{text}</div>
			</motion.div>
		);
	}
};
