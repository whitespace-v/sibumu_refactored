...
const DelivaryComponentAbout = (props) => {
    const variantImg = {
        visible:{
            opacity: 1,
            y: 0,
    ...
    return (
        <motion.div className={props.element.style_delivery} whileInView={'visible'} initial={'notVisible'}
            ...
            <motion.div className="delivery_elem_about" variants={variantImg} transition={()=>transition_option(props.element.id)}>
                <div className="picture_object_about">
                    <img className="scooter" alt="scooter_image" src={props.element.img}/>
                </div>
                <div className="text_delivery_about">{props.element.text}</div>
            </motion.div>
        </motion.div>
    );
};

export default DelivaryComponentAbout;