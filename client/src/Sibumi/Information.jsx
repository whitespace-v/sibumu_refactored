...
import img_logotype from '../picture_izBrazzerie/sibumi.png'
import mobileBackground from "../picture_izBrazzerie/sibumi_mobile.jpg"
import { motion } from 'framer-motion'
import sibumiVideo from "../videos/sibumiVideo.mp4"
const Information = ({ lines }) => {
	...
	var background = <img className='video_info' src={mobileBackground} alt='background' style={{ maxHeight: "100%", maxWidth: "50%" }} />
	...
	if (screenWidth >= 700) {
		background = <video className="video_info" autoPlay loop muted>
			...
			<source className='video_mp4' src={sibumiVideo} type='video/mp4' />
		</video>
	}
	...
	return (
		<div className="background_and_texts">
			...
			<motion.div className="text_on_img" whileInView={'visible'} initial={'notVisible'}
				...
				<motion.div variants={variantTitle} transition={{ duration: 1 }}>
					<Link className="order-delivery" to="/Sibumii">
						Заказать доставку
					</Link>
				</motion.div>
			</motion.div>
